...
describe('getDefaultRule', () => {
  it('should return the default rule', () => {
    ...
    expect(rulesMatcherUtils.getDefaultRule(['test'])).toEqual({
      op: '$in',
      value: ['test']
    })
    ...
    expect(
      rulesMatcherUtils.getDefaultRule({ name: 'John', value: 'test', op: '$in' })
    ).toEqual({
      name: 'John',
      op: '$in',
      value: 'test'
    })
    expect(rulesMatcherUtils.getDefaultRule(undefined)).toEqual({
      op: '$eq',
      value: undefined
    })
  })
